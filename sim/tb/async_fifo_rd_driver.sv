...
class async_fifo_rd_driver extends async_fifo_driver;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever 
          begin
              if (r_item != null)
                begin
                    `uvm_info(m_driver_name, "Got item from write driver", UVM_HIGH)
                    drive_item(r_item);
                end
    ...
    virtual task drive_item(fifo_seq_item m_item);
         ...
         for (int i = 0; i < m_item.data_depth; i++) 
           begin
               `uvm_info(m_driver_name, $sformatf("Wait for empty to drop"), UVM_HIGH)
